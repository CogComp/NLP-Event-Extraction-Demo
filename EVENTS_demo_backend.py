...
def getTEMPORAL_OLD(eventsOutput):
    ...
    return res_out_json # {} # res_out.json()
'''
def getTEMPORAL(eventsOutput, text=None):  #add 'text' argument for checking if the Temporal annotations of the text has already existed.
    global cache_EE
    lang="eng"
    hash_value = hashlib.sha1(text.encode()).hexdigest()
...
'''
def processTEMPORAL(myTabularView,eventsOutput,text=None):
    annjson = getTEMPORAL(eventsOutput,text)
    #print("====")
    #print(annjson)
    #print("====")
    if "tokens" in annjson:
...
def doProcess(myTabularView, text=None, anns=None):
    ...
    annjsonEvents = initView(myTabularView, text)
    ...
    for ann in anns:
        ...
        if hasEvents:
            if ann == "TEMPORAL" and eventsOutput is not None: 
                ...
                temporalOutput, temporalView = processTEMPORAL(myTabularView, annjsonEvents, text)
                print("-- temporalView --")
